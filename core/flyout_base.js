...
Blockly.Flyout.prototype.drawHighlightBox = function(ids) {
  for ( var i=0; i< ids.length; i++ ) {
    ...
    {
      'd': Blockly.utils.getBoundingPath(block,null),
      'class': 'blocklyPath blocklyBlockBackground',
      'stroke': 'black',
      'style': 'stroke-width: 5px',
      'transform': 'translate('+(block.getBoundingRectangle().topLeft.x-15) * this.workspace_.scale+','+(block.getBoundingRectangle().topLeft.y-this.getScrollPos()-5) * this.workspace_.scale+')',
      'fill-opacity': '0',
      'fill': 'black'
    },
    this.svgGroup_));
